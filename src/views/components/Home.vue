  ...
  <v-container >
    ...
    <v-card class="table-card mb-6" elevation="3">
      <v-card-title class="table-header flex">
        <v-row>
          <v-icon left color="blue" class="mr-2">mdi-format-list-bulleted</v-icon>
          O'quvchilar Ro'yxati
          <v-spacer></v-spacer>
          <v-chip small color="blue" text-color="white" class="ml-2">
      ...
      <v-data-table :headers="headers" :items="filteredStudents" :items-per-page="10" class="elevation-0" :footer-props="{
        ...
        <template v-slot:item.teacher.name="{ item }">
          <div class="d-flex align-center">
            <v-avatar size="30" color="blue lighten-3" class="mr-2">
              <span color="blue" class="white--text">{{
        ...
        <template v-slot:item.phone="{ item }">
          <div class="d-flex align-center">
            <v-icon small color="blue" class="mr-1">mdi-phone</v-icon>
        ...
        <template v-slot:no-data>
          <v-card flat class="text-center pa-5 mt-4">
            <v-img alt="No data" max-height="120" contain class="mx-auto mb-4"></v-img>
    ...
    formatPhone(phone) {
      ...
      if (cleaned.length === 12 && cleaned.startsWith("998")) {
        return `+${cleaned.substring(0, 3)} ${cleaned.substring(
          3,
          5
        )} ${cleaned.substring(5, 8)}-${cleaned.substring(8, 10)}-${cleaned.substring(
          10,
          12
        )}`;
      }
